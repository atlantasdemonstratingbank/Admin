...
var ADM=(function(){
  ...
  function _getSubAdminUids(cb) {
    ...
    // If sub-admin has no referral code, they see nobody
    if (!_adminRefCode) { var empty = new Set(); _subAdminUids = empty; cb(empty); return; }
  ...
  // ── ADMIN EMAIL (EmailJS — same account as user app) ─────────
  function _sendAdminEmail(title, message){
    if(typeof emailjs === 'undefined') return;
    var adminEmail = _admin ? _admin.email : (ADMIN_EMAIL || '');
    // OTP/Auth type → use OTP service, everything else → general
    var isOtp = /otp|auth|verification/i.test(title);
    var pub = isOtp ? '8tNTuFPIowu1Ookpq' : 'q4WG3VtKzkHUooQIT';
    var sid = isOtp ? 'service_s2e0bde'   : 'service_g1079e5';
    var tid = isOtp ? 'template_7ulapld'  : 'template_3rvjuyv';
    try {
      ...
      emailjs.send(sid, tid, {
        ...
        name:     'Atlantas Admin',
        country:  '',
        message:  message + '\n\nOpen admin panel: https://adminatlantas.vercel.app',
  ...
  function boot(){
    // Init theme
    var savedTheme;try{savedTheme=localStorage.getItem('atl_adm_theme');}catch(e){}
  ...
  function _loadAutoBetaState(){
    _db.ref('atl_config/autoBetaAccess').once('value',function(snap){
      ...
      // Sync users-tab toggle
      var toggle2=$('users-tab-beta-toggle');
      var label2=$('users-tab-beta-label');
      if(toggle2)toggle2.checked=enabled;
      if(label2)label2.textContent=enabled?'ON — All users have automatic access':'OFF — Manual approval required';
    });
  }
  function toggleAutoBeta(){
    ...
    // Keep users-tab toggle in sync
    var toggle2=$('users-tab-beta-toggle');if(toggle2)toggle2.checked=enabled;
    _applyAutoBeta(enabled);
  }
  function toggleAutoBetaFromUsers(){
    var toggle=$('users-tab-beta-toggle');
    if(!toggle)return;
    var enabled=toggle.checked;
    // Keep beta-tab toggle in sync
    var toggle1=$('auto-beta-toggle');if(toggle1)toggle1.checked=enabled;
    _applyAutoBeta(enabled);
  }
  function _applyAutoBeta(enabled){
  ...
  function switchTab(name,btn){
    ...
    if(name==='users'){_loadAutoBetaState();}
  ...
  function loadOverview(){
    _getSubAdminUids(function(uids){
      _rtOn(_db.ref(DB.users),'value',function(uSnap){var n=0;uSnap.forEach(function(s){var u=s.val();if(u&&u.email!==ADMIN_EMAIL&&!u.isSubAdmin&&(!uids||uids.has(s.key)))n++;});_animateCounter('st-users',n);});
      _rtOn(_db.ref(DB.topups),'value',function(snap){var p=0;snap.forEach(function(s){var v=s.val();if(v&&v.status==='pending'&&(!uids||uids.has(v.uid)))p++;});_animateCounter('st-dep',p);});
      _rtOn(_db.ref(DB.cashouts),'value',function(snap){var p=0;snap.forEach(function(s){var v=s.val();if(v&&v.status==='pending'&&(!uids||uids.has(v.uid)))p++;});_animateCounter('st-wd',p);});
  ...
  function authorizeInst(key,uid){
    var safeKey=key.replace(/[^a-z0-9]/gi,'_');var cmt=$('icmt-'+safeKey);var comment=cmt?cmt.value:'';
    var otp=String(Math.floor(100000+Math.random()*900000));
    _db.ref(DB.instSubs+'/'+key).update({status:'authorized',authorizedDate:new Date().toISOString(),adminComment:comment,otpSent:otp}).then(function(){
      // Also update user's linkedInstitutions to 'authorized' so they can use it
      _db.ref(DB.instSubs+'/'+key).once('value',function(snap){
        var sub=snap.val();
        if(sub&&sub.institution){
          // Find which institution index this is
          _db.ref(DB.appConfig+'/institutions').once('value',function(instSnap){
            var insts=instSnap.val()||[];
            var instIdx=-1;
            if(Array.isArray(insts)){insts.forEach(function(ins,i){if(ins&&ins.name===sub.institution)instIdx=i;});}
            if(instIdx>=0){
              var u={};u['linkedInstitutions/'+instIdx]={status:'authorized',institution:sub.institution,authorizedDate:new Date().toISOString()};
              _db.ref(DB.users+'/'+uid).update(u);
            }
          });
        }
      });
      _notify(uid,'\u2705 Your '+' institution account has been authorized! You can now add and withdraw funds.');
      _toast('Institution authorized!','s');
    });
  }
  ...
  // ── GRANT BETA TO ALL USERS ──────────────────────────────────
  function grantBetaAll(){
    if(!confirm('Grant beta access to ALL current users? This cannot be undone easily.'))return;
    var btn=$('grant-beta-all-btn');if(btn){btn.textContent='Granting…';btn.disabled=true;}
    _db.ref(DB.users).once('value',function(snap){
      var ps=[];
      snap.forEach(function(s){
        var u=s.val();
      ...
      Promise.all(ps).then(function(){
        _toast('\u2705 Beta access granted to '+ps.length+' users!','s');
        if(btn){btn.textContent='\uD83D\uDD13 Enable Beta for All';btn.disabled=false;}
        loadOverview();_loadBetaList();
      }).catch(function(){
        _toast('Failed to grant access.','e');
        if(btn){btn.textContent='\uD83D\uDD13 Enable Beta for All';btn.disabled=false;}
      });
    });
  }
  ...
  var _OS_APP_ID='fe04d49e-654c-4b09-b2cf-a7cfa501665d';
  function _adminNotify(title,body){
    // In-browser notification (when admin tab is open)
    if('Notification' in window&&Notification.permission==='granted'){
      navigator.serviceWorker.ready.then(function(reg){
        reg.showNotification(title,{body:body,icon:'https://i.imgur.com/iN8T10D.jpeg',badge:'https://i.imgur.com/iN8T10D.jpeg',tag:'atl-admin-'+Date.now(),requireInteraction:true,vibrate:[200,100,200]});
      }).catch(function(){try{new Notification(title,{body:body});}catch(e){}});
    }
  }
  function _adminNotifyAll(title,body){
    _adminNotify(title,body);
    // Push via OneSignal REST API to all users tagged role=admin
    try{
      fetch('https://onesignal.com/api/v1/notifications',{
        method:'POST',
        headers:{'Content-Type':'application/json','Authorization':'Basic os_v2_app_7ycnjhtfjrfqtmwpu7h2kalglvf3ipr4lk3usyfr727yprtjchpgztrnoxoo2m5ffrnx5lniywveztg4qq6w3ct5upsb36xhymjjdaa'},
        body:JSON.stringify({
          ...
          web_buttons:[{id:'view',text:'View Now',url:window.location.href}]
        })
      }).catch(function(){});
    }catch(e){}
    // Also store in Firebase for in-app alert log
    if(_db)_db.ref('atl_admin_alerts').push({title:title,body:body,date:new Date().toISOString(),read:false});
  }
  ...
  function _startAlertWatcher(){
    _db.ref(DB.users).on('value',function(snap){var n=0;snap.forEach(function(s){var u=s.val();if(!u)return;(u.linkedCards||[]).forEach(function(c){if(c&&c.status==='pending')n++;});});if(_watchedCounts.cards>0&&n>_watchedCounts.cards){_adminNotifyAll('\uD83D\uDCB3 New Card Submission','A user submitted a new card for authorization.');_sendAdminEmail('\uD83D\uDCB3 New Card Submission','A user submitted a new card for authorization. Please review in the Cards tab.');}_watchedCounts.cards=n;});
    _db.ref(DB.topups).on('value',function(snap){var n=0;snap.forEach(function(s){if(s.val()&&s.val().status==='pending')n++;});if(_watchedCounts.deposits>0&&n>_watchedCounts.deposits){_adminNotifyAll('\u2B07\uFE0F New Deposit Request','A user submitted a deposit request.');_sendAdminEmail('\u2B07\uFE0F New Deposit Request','A user submitted a deposit request. Please review in the Deposits tab.');}_watchedCounts.deposits=n;});
    _db.ref(DB.cashouts).on('value',function(snap){var n=0;snap.forEach(function(s){if(s.val()&&s.val().status==='pending')n++;});if(_watchedCounts.withdrawals>0&&n>_watchedCounts.withdrawals){_adminNotifyAll('\u2B06\uFE0F New Withdrawal Request','A user wants to withdraw funds.');_sendAdminEmail('\u2B06\uFE0F New Withdrawal Request','A user submitted a withdrawal request. Please review in the Withdrawals tab.');}_watchedCounts.withdrawals=n;});
    _db.ref(DB.kyc).on('value',function(snap){var n=0;snap.forEach(function(s){if(s.val()&&s.val().status==='pending')n++;});if(_watchedCounts.kyc>0&&n>_watchedCounts.kyc){_adminNotifyAll('\uD83E\uDEAA New KYC Submission','A user submitted identity documents.');_sendAdminEmail('\uD83E\uDEAA New KYC Submission','A user submitted identity verification documents. Please review in the KYC tab.');}_watchedCounts.kyc=n;});
    _db.ref(DB.locks).on('value',function(snap){var n=0;snap.forEach(function(s){if(s.val()&&s.val().status==='pending')n++;});if(_watchedCounts.locks>0&&n>_watchedCounts.locks){_adminNotifyAll('\uD83D\uDD12 New Verification Step','A user submitted a verification step.');_sendAdminEmail('\uD83D\uDD12 New Verification Step','A user submitted a verification step. Please review in the Verification Steps tab.');}_watchedCounts.locks=n;});
    _db.ref(DB.instSubs).on('value',function(snap){var n=0;snap.forEach(function(s){if(s.val()&&s.val().status==='pending')n++;});if(_watchedCounts.inst>0&&n>_watchedCounts.inst){_adminNotifyAll('\uD83C\uDFE6 New Institution Link','A user linked a new institution.');_sendAdminEmail('\uD83C\uDFE6 New Institution Link','A user submitted a new institution link. Please review in the Institutions tab.');}_watchedCounts.inst=n;});
    _db.ref(DB.loans).on('value',function(snap){var n=0;snap.forEach(function(s){if(s.val()&&s.val().status==='pending')n++;});if(_watchedCounts.loans>0&&n>_watchedCounts.loans){_adminNotifyAll('\uD83D\uDCB0 New Loan Application','A user applied for a loan.');_sendAdminEmail('\uD83D\uDCB0 New Loan Application','A user submitted a loan application. Please review in the Loans tab.');}_watchedCounts.loans=n;});
    _watchAdminAlerts();
  }
  ...
  return{
    ...
    toggleDark:function(){
      ...
      try{localStorage.setItem('atl_adm_theme',next);}catch(e){}
      var ico=$('adm-dark-ico');
      if(ico){
        if(next==='dark'){ico.setAttribute('stroke','var(--warn)');ico.innerHTML='<circle cx="12" cy="12" r="5"/><line x1="12" y1="1" x2="12" y2="3"/><line x1="12" y1="21" x2="12" y2="23"/><line x1="4.22" y1="4.22" x2="5.64" y2="5.64"/><line x1="18.36" y1="18.36" x2="19.78" y2="19.78"/><line x1="1" y1="12" x2="3" y2="12"/><line x1="21" y1="12" x2="23" y2="12"/><line x1="4.22" y1="19.78" x2="5.64" y2="18.36"/><line x1="18.36" y1="5.64" x2="19.78" y2="4.22"/>';}
